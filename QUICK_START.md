# 快速开始指南

## 🚀 快速部署（推荐）

```bash
# 1. 构建前端
./build-frontend.sh

# 2. 启动所有服务
docker compose up -d

# 3. 访问
open http://localhost:8000
```

默认管理员账号：
- 用户名: `admin`
- 密码: `admin`（请在 `.env` 中修改）

---

## 📖 使用流程

### 管理员操作

1. **登录管理后台**
   - 访问 http://localhost:8000
   - 使用管理员账号登录

2. **添加Twitch用户到Bot**
   - 点击"添加用户"
   - 系统会调用C# bot的 `--add-account` 命令
   - 查看Docker日志获取Twitch授权链接和代码
   - 将链接和代码发给要添加的用户
   - 用户完成授权后，C# bot会自动添加到 config.json

3. **管理用户**
   - 启用/禁用用户的bot
   - 删除用户
   - 查看用户状态

### 普通用户操作

1. **登录查看自己信息**
   - 访问 http://localhost:8000
   - 点击"通过Twitch登录"
   - 完成Twitch OAuth授权
   - 前提：管理员已经添加了你的账户到bot

2. **查看Dashboard**
   - 查看bot运行状态
   - 查看当前Campaign
   - 查看观看进度

3. **配置管理**
   - 设置优先游戏列表
   - 保存配置

4. **查看日志**
   - 实时查看bot日志
   - 分页浏览历史日志

---

## 🔧 开发模式

### 后端

```bash
cd web-backend
python -m venv venv
source venv/bin/activate
pip install -r requirements.txt
uvicorn app.main:app --reload --port 8000
```

### 前端

```bash
cd web-frontend
npm install
npm run dev  # 运行在 8080，自动代理API到8000
```

---

## 🐛 故障排除

### 问题：无法访问前端

**检查**：
```bash
# 确认前端已构建
ls -la web-backend/static/

# 如果没有，运行构建
./build-frontend.sh
```

### 问题：管理员登录失败

**解决**：
```bash
# 检查环境变量
cat .env

# 如果没有.env，创建一个
echo "ADMIN_USERNAME=admin" > .env
echo "ADMIN_PASSWORD=admin" >> .env
echo "JWT_SECRET_KEY=$(openssl rand -hex 32)" >> .env
```

### 问题：添加用户后看不到

**原因**：C# bot的 `--add-account` 是交互式命令

**查看输出**：
```bash
docker compose logs -f drop
```

你应该看到Twitch授权链接和代码，类似：
```
Please visit: https://www.twitch.tv/activate
Enter code: ABCD-EFGH
```

---

## 📁 关键文件位置

```
/root/twitch/
├── config.json          # Bot配置（C# bot管理）
├── logs/                # Bot日志
│   ├── {username}.txt
│   └── system-log.txt
├── web-backend/
│   ├── app.db           # Web数据库（管理员账户）
│   └── static/          # 前端构建文件
└── web-frontend/
    └── dist/            # 构建输出
```

---

## 🔐 安全提示

生产环境必须：
1. 修改管理员密码
2. 更换 JWT_SECRET_KEY
3. 使用 HTTPS
4. 限制文件权限

---

## 📚 更多文档

- `README.md` - 完整项目说明
- `ARCHITECTURE.md` - 系统架构设计
- `DEPLOYMENT.md` - 部署指南
- `CLAUDE.md` - 开发者参考

---

## ⚡ 常用命令

```bash
# 查看日志
docker compose logs -f

# 重启服务
docker compose restart

# 停止服务
docker compose down

# 更新前端后重启
./build-frontend.sh && docker compose restart web

# 手动添加账户（通过bot）
./add_account.sh
```
